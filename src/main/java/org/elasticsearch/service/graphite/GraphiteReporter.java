...
public class GraphiteReporter {
    ...
    public GraphiteReporter(String host, int port, String prefix, NodeIndicesStats nodeIndicesStats,
                            ...
                            Pattern graphiteInclusionRegex, Pattern graphiteExclusionRegex, String localNodeName, Boolean master) {
        this.localNodeName = localNodeName.replaceAll("[\\s\\.]","");
        this.master = master;
        this.host = host;
        this.port = port;
        this.prefix = prefix;
    ...
    private void sendNodeThreadPoolStats(ThreadPoolStats threadPoolStats) {
        String type = buildMetricName(String.format("node.%s.threadpool", this.localNodeName));
    ...
    private void sendNodeTransportStats(TransportStats transportStats) {
        String type = buildMetricName(String.format("node.%s.transport", this.localNodeName));
    ...
    private void sendNodeProcessStats(ProcessStats processStats) {
        String type = buildMetricName(String.format("node.%s.process", this.localNodeName));
    ...
    private void sendNodeOsStats(OsStats osStats) {
        String type = buildMetricName(String.format("node.%s.os", this.localNodeName));
    ...
    private void sendNodeNetworkStats(NetworkStats networkStats) {
        String type = buildMetricName(String.format("node.%s.network.tcp", this.localNodeName));
    ...
    private void sendNodeJvmStats(JvmStats jvmStats) {
        String type = buildMetricName(String.format("node.%s.jvm", this.localNodeName));
    ...
    private void sendNodeHttpStats(HttpStats httpStats) {
        String type = buildMetricName(String.format("node.%s.http", this.localNodeName));
    ...
    private void sendNodeFsStats(FsStats fs) {
        ...
        while (infoIterator.hasNext()) {
            String type = buildMetricName(String.format("node.%s.fs", this.localNodeName)) + i;
    ...
    private void sendIndexShardStats() {
        for (IndexShard indexShard : indexShards) {
            //String type = buildMetricName("indexes.") + String.format("node.%s.http", this.localNodeName) + "." + indexShard.shardId().index().name() + ".id." + indexShard.shardId().id();
            String type = buildMetricName("indexes.") + indexShard.shardId().index().name() + "." + this.localNodeName + ".shard." + indexShard.shardId().id();
    ...
    private void sendNodeIndicesStats() {
        String type = buildMetricName(String.format("node.%s", this.localNodeName));
