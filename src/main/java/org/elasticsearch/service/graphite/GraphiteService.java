            ...
            while (!closed) {
                ...
                if (isClusterStarted && node != null) {
                    ...
                    GraphiteReporter graphiteReporter = new GraphiteReporter(graphiteHost, graphitePort, graphitePrefix,
                            nodeIndicesStats, indexShards, nodeStats, graphiteInclusionRegex, graphiteExclusionRegex, localNodeName, node.isMasterNode());
                    
                    graphiteReporter.run();
                } else {
                    if (node != null) {
                        logger.debug("Cluster is not started, not triggering update");
                    }
                }
                ...
                if (isClusterStarted && node != null && node.isMasterNode()) {
                    ...
                    GraphiteReporter graphiteReporter = new GraphiteReporter(graphiteHost, graphitePort, graphitePrefix,
                            nodeIndicesStats, indexShards, nodeStats, graphiteInclusionRegex, graphiteExclusionRegex, localNodeName, true);
                    graphiteReporter.run();
                } else {
                    if (node != null) {
